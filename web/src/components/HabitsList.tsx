...
export function HabitsList({date, onCompletedChanged}: HabitsListProps) {
    ...
    async function handleToggleHabit(habitId: string) {
        ...
        const isHabitAllreadyCompleted = habitsInfo!.completedHabits.includes(habitId)

        let completedHabits: string[] = []

        if(isHabitAllreadyCompleted) {
            ...
            {habitsInfo?.possibleHabits.map(habit => {
                return (
                    ...
                    >
                        ...
                        <span className="font-semibod text-xl text-white leading-tight group-data-[state=checked]:line-through group-data-[state=checked]:text-zinc-400">
                            {habit.title}
                        </span>
                    </Checkbox.Root>
                )
